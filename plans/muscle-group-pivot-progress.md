...
## Phase 7 — Session runtime + state machine `[x]`

> **2026-05-21:** Pure reducer in `src/lib/workout/session-machine.ts` covers all 12 states + 15 events (idle/prompt-selecting/recording/transcribing/scoring/score-reveal/walking/day-complete-prompt/graduation-rep/day-complete/paused; START/PICK_PROMPT/FINISH_RECORDING/TRANSCRIBE_DONE/SCORE_PROGRESS/SCORE_DONE/FAIL_SCORE/ADVANCE/WALK_DONE/ACCEPT_GRADUATION/SKIP_GRADUATION/GRADUATION_DONE/PAUSE/RESUME/NETWORK_DROP|RECONNECT). Provider in `src/lib/workout/use-workout-session.tsx` wires it to React + side effects: auto-advance 5s after score-reveal, mascot-walk timer matched to MASCOT_TIMINGS.walk_ms, visibilitychange → PAUSE during recording, online/offline → NETWORK_DROP/RECONNECT, server persistence via `updateWorkoutSessionState` (300ms debounced). Server actions in `src/server/actions/workout-session.ts`: `updateWorkoutSessionState`, `getActiveWorkoutSession`, `completeWorkoutSession`, `recordGraduationRep`, `tagWorkoutRep` (sets reps.exercise_id + muscle_group_day_id + score_failure_flag and increments completed_reps). IndexedDB buffer at `src/lib/workout/network-buffer.ts` with memory-store fallback for SSR/RN ports. RepControls now mounts `<RepSurface>` for the active rep + graduation rep (mode='daily_workout'), with `onComplete` calling `tagWorkoutRep` then `onRepScored` → SCORE_DONE/FAIL_SCORE. WorkoutShell exposes data-phase + data-mascot-state attributes for debugging. mascotState derivation includes `stumbling` on score failure. 46 reducer tests in `tests/workout-session-machine.test.ts` cover every transition + every event from every state. Wired into `npm test`. Build green, /workout=8.92 KB. **Deferred to manual verification:** end-to-end full-day run with pause-mid-rep, force-kill scoring, real-device airplane-mode test — sandboxed environment can't drive mic + network state.
...
## Phase 8 — Scoring integration `[x]`

> **2026-05-21:** Exercise context threaded end-to-end from RepSurface → /api/score(/stage1/stage2)/score-internal → scoreStage1/2 → telemetry. Migration 0022 adds `exercise_id` + `muscle_group_day_id` + `is_graduation_rep` to `scoring_telemetry`. New `src/lib/ai/muscle-group-exercises.ts` exports `EXERCISE_RUBRIC_HINTS` (covers Headline First, Word Budget, Explain Like I'm 12 (clarity); 3-Point Rule, Bottom-Line First, Signpost First (structure); Kill the Filler, 30-Second Rule, No Hedging, Single Sentence (conciseness); Claim and Proof, So-What Test, Steel Man (thinking_quality); Strategic Pause, Silence Over Filler, Beat the Buzzer (pacing); Monotone Breaker, Downward Landing, Authority Voice (tone)), `getExerciseScoringContext(id)`, `renderExerciseXmlBlock`, `tryExerciseFastFail`. prepareContext now injects `<exercise name="..." dimension="..." rule="..."/>` XML in the user prompt when `input.exerciseId` is set; Stage 2 user message gets a single operator-facing constraint sentence appended. Kill-the-Filler fast-fails conciseness to 25 when filler rate > 4/min; 30-second-rule fast-fails when duration > 45s. reps.ts (insertPendingRep + saveRep) accepts exerciseId/muscleGroupDayId/isGraduationRep + writes them at save time. writeScoringTelemetry accepts the three new fields. promoteRepToExemplar stamps `tags.exercise_id`. /api/score, /api/score/stage1, /api/score/stage2, /api/score-internal all accept the new body params (body wins over rep-row hydration to avoid the Phase-7-tag race). RepSurface accepts the props + forwards via insertPendingRep + saveRep + scoreBody. **Critical invariant preserved:** every exerciseId-specific code path is guarded — legacy reps (no exerciseId) get byte-identical scoring inputs to today. **Deferred to real-API runs (sandbox can't execute):** 48-rep calibration replay, --exercise-id flag on phase-baseline.mjs, /ops/calibration per-exercise drift view, RAG dimension filter (Phase 8.5 candidate). Build green, 46 tests pass.
