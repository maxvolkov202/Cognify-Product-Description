...
| 2026-07-07 | **Phase 16 — full E2E re-test + production readiness (Max: "test everything end to end then lets get it to production").** Test matrix ALL GREEN: 13 unit suites, 38/38 contracts, 13 unauthed, 34 sweep+probe, Lab+BaR live loops, and the FIRST-EVER whole-day live run (3 exercises → graduation offer → Day Complete). That run caught 2 real bugs: W-1 closed the day on the 3rd FIRST attempt (now closes on the ending retry) and showLanding classified day-complete as "at-rest" so the §5.7 celebration was UNREACHABLE live. Visual tour (16 screenshots, 390px+desktop, demo acct) caught: streak MORNING-DEATH (anchor-alive walk counted today as missed → every Mon-Fri user woke to streak 0; walk also spent a freeze on the in-progress day), stale v1 copy (4 reps/10 min → 3 exercises/6 min), 390px label truncation, marketing still selling "Scenario Training". Prod-readiness audits (2 agents): cron auth hardened (spoofable x-vercel-cron removed; fail-closed), rollover cron hourly+maxDuration, /api/health details gated behind OPS/CRON secret, deleteAccount auth-orphan fixed, robots/sitemap/favicon, @vercel/analytics, .env.example regenerated (45 vars, 4 tiers), verify-prod-migrations extended to 0028-0039, seed/backfill host interlocks, **`plans/prod-promotion-runbook.md`** (verified end-to-end). Commits 3420e4f2→d45878db. AWAITING MAX: push/PR decision, flag rollout shape, launch provider, external services (Resend/Upstash/uptime). |
...
- Per-IP limits loosened for shared-NAT users (Max: "don't rate limit too hard"): weekly-narrative 10→60/min, support 5→20/10min, bug-reports 5→20/h. Per-user training-loop limits unchanged; limiter still fails open on any Redis error.
- `.github/workflows/uptime.yml`: 15-min /api/health probe (GitHub failure email = the alert; activates when merged to main).
...
- `npx vercel deploy --prod` succeeded after fixing vercel.json for the Hobby-plan daily-cron limit (rollover `0 9 * * *`, committed-day-reminder `0 16 * * *`; both routes were designed for hourly — Vercel Pro restores `0 * * * *`).
- Live smoke on https://cognify-v2-neon.vercel.app: public /api/health `{ok:true}`; Bearer-authed detailed health (db reachable 153ms, provider=openai, both keys present); /api/score/health live probe ok (1.7s); `/`, `/signin`, `/workout`, robots.txt all 200; committed-day-reminder dryRun ok.
- Prod env: 5 v3 flags true, AI_PROVIDER=openai, NEXT_PUBLIC_APP_URL set; FF_DETERMINISTIC_SIGNALS/FF_BAND_ANCHORS/FF_SUBSKILL_UI=false (tested-config parity). `.env.prod-temp` deleted.
- Cron fix committed (4dd8f097) and pushed to feat/prd-v3-wave3 (PR #2).
- Remaining human steps: merge PR #1 then PR #2; provision Resend/Upstash/uptime monitor; on Anthropic credit re-up remove AI_PROVIDER=openai and re-baseline; consider Vercel Pro for hourly crons.
