...
- **2026-09-01 — Prosody v2 Phase 0: yardsticks secured** (`chore/labeling-packet-resign`).
  - **`build-packet.mjs --resign` built first** (plan §3.4): reads the frozen `sample.json` +
    `model-scores.hidden.json`, refreshes signed audio URLs only, rewrites sample + both sheets;
    no DB connection, no resample; strict signing (aborts rather than write dead links);
    `--ttl-days` supported (default 7).
  - **Packet re-signed** ahead of the ~09-04 expiry. Verify: byte-compare before/after shows the
    11 signed URLs as the ONLY diff in `sample.json` / `labeling-sheet-A.csv` / `-B.csv`
    (model-scores.hidden.json, strata.md, README byte-identical); all 11 links HEAD-check 200;
    new expiry 2026-09-09.
  - **Fixture tone mini-sheet built** (`build-fixture-minisheet.mjs`): the 15 audio-grading
    fixtures uploaded to `rep-audio/fixtures/tone-mini-2026-09/` under masked seeded-shuffle
    names (filenames encode flat/expressive/rushed — direct links would unblind),
    `fixture-mini-sheet-A.csv` (Max) / `-B.csv` (Owen) + `fixture-mini-key.hidden.json`
    (gitignored). Verify: 15 rows, A==B, all 15 links HEAD-check 200 (exp 2026-09-09), no style
    strings leak into the sheet. Storage writes were additive under the `fixtures/` prefix only;
    DB untouched.
  - **Found while verifying: the packet data files were tracked in the public repo.** PR #78's
    commit (`f38c43e9`, 08-28) force-added `sample.json`, both labeling sheets,
    `model-scores.hidden.json`, and `strata.md` past the gitignore rule — real user transcripts,
    the hidden model scores, and signed audio URLs (live until ~09-04) have been public on
    GitHub since 08-28. This PR untracks them (`git rm --cached`; ignore rule confirmed to catch
    them once untracked) so they leave HEAD, but **git history still contains them — Max
    decision needed:** make the repo private, or scrub history (filter-repo + force push), or
...
- **2026-09-02 — Prosody v2 Phase 3: tone-core v2** (`feat/tone-core-v2`).
  - **Core rewrite** (`src/lib/scoring/tone-core.ts`): windowed monotone is an INDEPENDENT
    penalty (in-system p50 0.41 → −1.6, p90 0.88 → −12; full-flat −14); upspeak prefers the
    aligned ratio (silence-heuristic noise floor raised to 0.2 — TTS clips with ZERO aligned
    upspeak read up to 0.25 raw); NEW finalFall bonus (0→0, 0.8→+6 — the flat≈0/expressive
    0.5–1.0 separator); variety curve retuned to v2 extractions; rms dynamics re-anchored to the
    measurement's real scale (v1's 2–10 dB anchors were an order off — in-system p50 22 dB);
    articulation halved (±3) and re-anchored to observed p50 0.13. blendToneWithModel ±10 and
    the divergence feedback swap unchanged (feedback text updated to v2 signals, plain language).
  - **Feature vectors committed** as `tests/fixtures/audio-grading/features-v2.json` (deployed
    worker v2 output — unit tests need no audio/worker). 57-assertion tone-core suite including
    a per-script GF1 block.
  - **Gate table:**
    | gate | result |
    |---|---|
    | GF1 (fixture vectors, deterministic) | **PASS**: flat_max 20 (≤45) · expressive_min 68 (≥65) · min separation 48 (≥25), 5/5 pairs |
    | GF2 (re-check) | **no exposure**: the branch touches post-processing + tests + calibration data only — zero scoring-prompt bytes (verified by inspection of the diff) |
    | GP1 (in-system, final-v2 features fresh-extracted for all 115 audio reps) | **PASS**: real reps n=30 sd 15.9, mode share 10%, range 21–77 (vs the LLM's clumped 38–75); all reps sd 22.4, mode 15% |
    | In-product (dev, flag ON, local v2 worker, re-promoted bank) | **calibrate-audio-tone 15/15, 0 warnings**: flat 30 across scripts (core 20 + blend), expressive 64–75, tone-pair separations 34–45 |
  - **Bank re-promoted** (promote-audio-reps.mjs): the band-competent "upspeak specimen"
    carve-out RETIRED — v1's upspeakRatio 0.5 on that clip was a segmentation artifact refuted by
    v2 (all tails fall); the clip is a normal expressive rep (min 60) and its pair re-enters the
    separation gates. The upspeak-penalty regression lives in tone-core.test.ts as a
    deterministic assertion.
  - **/code-review: 10 findings, all addressed pre-merge.** The three that mattered most: (1)
    v1-shaped features (std-derived monotoneRatio — v1 cache rows, post-revert extractions, and
    worker v2's own short-clip fallback) were double-charged by the independent monotone penalty
    → the worker now ships `monotoneWindowed` and the core charges the penalty only for truly
    windowed v2 values; (2) a single aligned declarative could swing tone ±25 off one tail →
    alignSegmentTails keeps v1's ≥2 floor; (3) buildToneFeedback thresholds contradicted the
    curves on the divergence path (the one place its text IS the user's feedback) → rewritten on
    the same cuts as the scoring curves, with coherence tests. Also: the std-3 range-lift cliff
    removed; aligned vs raw upspeak/finalFall get separate curves (raw noise band ≤0.3 uncharged,
    raw fall bonus damped to +3); dynamics weight cut to ±2 with an honest weak-proxy note; GF1's
    flat arm asserts the PRE-CLAMP raw (≤35) so the clamp can't hide erosion; features-v2.json is
    now regenerated by script (update-feature-vectors.mjs) from real worker output; the dead
    upspeakSpecimen consumer removed from calibrate-audio-tone; promote header/rationales made
    verifiable. GP1 re-verified post-fix: real sd 17.2, mode 7%. 67-assertion tone-core suite.
  - Prod behavior unchanged this phase: FF_TONE_PROSODY_CORE stays OFF in prod until Phase 6.
    Example reps re-seeded end-of-phase per §6 (batch tag phase3-tone-core: 15/15, all
    featureVersion 2, graded_from_audio 14/15 — same single cold-start race as every batch).
  - **Verify checklist for Max (hands-on, ~10 min):**
    1. `npm test` — the tone-core suite prints 67 assertions incl. per-script GF1.
    2. Local dev (flag defaults ON): record one deliberately FLAT rep and one animated rep on
       any prompt — flat should read tone ≤45 with feedback about pitch barely moving; animated
       ≥65. The Tone signals line shows `[toneCore: N (...); model: M]`.
    3. In the seeded account (e2e-harness@cognify.test, dev): open a `phase3-tone-core` batch
       rep — feedback renders, tone matches the clip's style.
    4. Prod is UNCHANGED this phase (flag off): spot-check any new prod rep's tone signals —
       no `[toneCore: ...]` tag should appear.
  - **Next:** Phase 4 (`feat/live-rep-metrics`) — FF_LIVE_REP_METRICS measured-delivery panel in
