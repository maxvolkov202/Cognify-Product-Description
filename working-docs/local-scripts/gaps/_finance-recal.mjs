...
// for the FINANCE vertical. Appends to scripts/exercise-catalog/v1/vertical/finance.json.
//
...
//   - Vertical-flavored: finance themes (forecasts, numbers, models, budgets,
//                       deals, audits, controls, investors) NOT vertical-locked
//                       scenes ("walk the audit committee through the goodwill
//                       impairment KAM"). Use finance vocab SPARINGLY.
//   - ≤180 chars, conversational, second-person.
//   - NO mechanic-prefix-in-prompt language wall: short prefixes only, and
//     never restate the mechanic as the entire prompt opening.
...
// Target user: FP&A analyst, controller, accountant, IB associate 1-3 yrs,
// junior research analyst. NOT CFOs.

import fs from "node:fs";

const CATALOG_PATH = "C:/Users/MaxVolkov/dev/cognify/scripts/exercise-catalog/v1/vertical/finance.json";
...
// Universal A pools — universal craft, no vertical flavor. 10 per exercise,
// one per goal. Worded as invitations to speak, not as exam questions.
const UNIVERSAL_A = {
  ...
  "Cut by Half": [
    ...
    "Talk about your week. Now make it half as long.",
  ...
  "Compare and Contrast": [
    ...
    "Compare a win you fought for to one that came easy.",
...
// Per-goal FINANCE-flavored seed pools.
// B: vertical-lensed universal — finance theme, no specific scene to invent.
// C: light scenario — a *kind* of finance moment anyone encounters, not a
//    specific company / committee / KAM.
// Vocabulary stays light: forecast, model, budget, number, deal, audit,
// controls, IC, investor, lender, treasury, accrual, working capital.
const GOAL_FLAVOR = {
  handling_objections: {
    bSeed: [
      "When someone challenges a number you stand by, where do you start?",
      "What's the right move when your model gets picked apart?",
      "How do you handle pushback on an assumption you can't fully prove?",
      "What changes a skeptic's mind faster — more detail or better framing?",
      "When does conceding a small point make a stronger case?",
    ],
    cSeed: [
      "Push back when someone says your forecast is too aggressive.",
      "Respond when a reviewer says your model is wrong — and they're half right.",
      "Push back on a request to change a number you don't think should change.",
      "Respond to 'this doesn't tie' without losing the room.",
      "Defend a methodology call when someone wants the easy answer.",
    ],
  },
  confidence: {
    bSeed: [
      "Where does a finance person's credibility actually come from?",
      "What does confidence sound like when you're presenting numbers you're unsure about?",
      "What's the difference between sounding sure and being sure?",
      "When does a junior on a deal team earn the room?",
      "What does quiet confidence look like in a review meeting?",
    ],
    cSeed: [
      "Open a Monday review like the numbers are yours and you know them cold.",
      "Walk into a meeting where you're the most junior person and own your slide.",
      "Take a question on a number you only just learned this morning.",
      "Present a result you're not 100% confident in — without flinching.",
      "Hold the room when someone senior disagrees with your view.",
    ],
  },
  thinking_on_the_spot: {
    bSeed: [
      "What do you do when someone asks for a number you don't have in front of you?",
      "How do you stay useful when the meeting takes the model somewhere new?",
      "How do you think out loud about a number without losing credibility?",
      "What's the right way to say 'I don't know' on the spot?",
      "When you get a cold question, what's your first move?",
    ],
    cSeed: [
      "Someone asks for a ratio you didn't bring. Answer anyway.",
      "Your screen-share crashes mid-review. Keep going.",
      "Get asked the question you hoped no one would ask. Answer.",
      "Get cornered on an assumption you didn't pressure-test. Respond.",
      "A senior pulls a thread you didn't expect. Stay with it.",
    ],
  },
  giving_feedback: {
    bSeed: [
      "What kind of feedback actually makes someone's model better?",
      "What's the difference between feedback that helps and feedback that nitpicks?",
      "How do you give feedback on a number without making it personal?",
      "When does honest feedback on someone's work do more harm than good?",
      "What's the best feedback you ever got on something you built?",
    ],
    cSeed: [
      "Give feedback on a model that's clean but missing the point.",
      "Tell a peer their slide doesn't land — without crushing them.",
      "Hand a draft back to someone who clearly rushed it.",
      "Tell a junior their assumptions are off — kindly.",
      "Give honest feedback on a deck you didn't love.",
    ],
  },
  negotiation: {
    bSeed: [
      "Where do you give and where do you hold when budgets are tight?",
      "What's worth negotiating on a number and what isn't?",
      "What do you trade for time when a deadline is impossible?",
      "How do you keep authority while still meeting people halfway?",
      "What's the move when both sides think their number is right?",
    ],
    cSeed: [
      "Push back on a budget cut without sounding defensive.",
      "Trade scope for time when a deadline won't move.",
      "Negotiate a number you think is too low — without burning the room.",
      "Hold your number when someone keeps asking for a better one.",
      "Find the middle on a forecast neither side fully believes.",
    ],
  },
  presenting: {
    bSeed: [
      "What makes a numbers update actually worth listening to?",
      "What separates a finance person who holds a room from one who doesn't?",
      "Why do most update meetings lose the room in the first minute?",
      "What's the opening line that makes people stop checking email?",
      "What does a great numbers walkthrough do in the first 30 seconds?",
    ],
    cSeed: [
      "Open a quarterly update so people stop checking their phones.",
      "Open a walkthrough of results you don't love.",
      "Open a deck to a room that's seen ten of these this month.",
      "Open a numbers review after a stretch of bad results.",
      "Open a meeting you're not sure you should be running.",
    ],
  },
  persuasion: {
    bSeed: [
      "What actually changes someone's mind about a number?",
      "Why does more detail almost never persuade a skeptic?",
      "What's the most persuasive thing you can do in a numbers meeting?",
      "What changes a decision-maker's mind when the math is contested?",
      "What persuades a senior more — your model or your judgment?",
    ],
    cSeed: [
      "Convince someone the conservative case is the right one to plan on.",
      "Convince a reviewer the right move is to revisit a number you both signed off on.",
      "Convince a peer your read of the trend is the right one.",
      "Convince someone your assumption is more defensible than theirs.",
      "Convince a room that the unsexy answer is the right one.",
    ],
  },
  storytelling: {
    bSeed: [
      "What's a true work moment that changed how you read numbers?",
      "What's a story you tell new hires about the job?",
      "What's a small thing that changed how you do your work?",
      "What's a story you tell about why this work matters to you?",
      "What's a moment on a deal or close that stayed with you?",
    ],
    cSeed: [
      "Tell a story about a number that turned out to be wrong — and what you did.",
      "Tell a story about a close that went sideways.",
      "Tell a story about catching something no one else saw.",
      "Tell a story about a review where you got humbled.",
      "Tell a story about a deal you stayed on long after you should have walked.",
    ],
  },
  asking_questions: {
    bSeed: [
      "What's a question that gets a real answer in a numbers review?",
      "What's the question you wish more people asked before signing off?",
      "What's the question worth asking on every deal or close?",
      "What's a question you stopped asking — and wish you hadn't?",
      "What's the best question you've ever asked in a review?",
    ],
    cSeed: [
      "Ask the question that gets a quiet reviewer to actually engage.",
      "Ask the question that surfaces the assumption no one wants to touch.",
      "Ask the question that turns 'fine' into a real answer.",
      "Ask the question that opens up what's really going on with the numbers.",
      "Ask the question that makes someone defend their work — without sounding hostile.",
    ],
  },
  explaining: {
    bSeed: [
      "What do finance people over-explain that they should just say?",
      "What's the difference between explaining a number and defending it?",
      "When does a long explanation hurt your credibility?",
      "Why do people tune out the second a number walkthrough starts?",
      "What's the simplest test that an explanation actually landed?",
    ],
    cSeed: [
      "Explain a number to someone who isn't in finance — and needs the answer.",
      "Explain a result to someone after the first try clearly didn't land.",
      "Explain why a number moved — to someone who's mostly looking for reassurance.",
      "Explain a technical accounting point in plain English.",
      "Explain why your answer is your answer, without going into the model.",
    ],
  },
};

// Mechanic frames — short prefixes only. NO long mechanic-prefix language
// walls. Many exercises get NO prefix; the universal A and seed phrasing
// already pulls the mechanic. Keep ≤25 chars when used.
const MECHANIC_FRAMES = {
  "Answer the Confusion": { b: "", c: "" },
  "Explain Like I'm 12": { b: "In plain words: ", c: "Like to a curious kid: " },
  "Headline First": { b: "Headline first — ", c: "Headline first — " },
  "Cut by Half": { b: "Long version, then half: ", c: "Long, then half: " },
  "The Word Budget": { b: "In thirty words: ", c: "Thirty words: " },
  "Kill the Filler": { b: "No um, no like — ", c: "No filler — " },
  "The Steel Man": { b: "Strongest version — ", c: "Steel-man it — " },
  "Strategic Pause": { b: "", c: "" },
  "Silence Over Filler": { b: "Let silence carry it — ", c: "Use silence, not filler — " },
  "Two-Beat Landing": { b: "Land it, then two beats — ", c: "Land it — two beats — " },
  "Warmth Switch": { b: "Start firm, end warm — ", c: "Switch firm to warm — " },
  "Prove It": { b: "Claim plus proof — ", c: "Claim plus proof — " },
  "One Point Only": { b: "One point only — ", c: "Stay on one — " },
  ...
  "One Idea Per Response": { b: "One idea — ", c: "Hold one idea — " },
  "Authority Voice": { b: "With weight — ", c: "With authority — " },
  "Beat the Buzzer": { b: "Ten seconds — ", c: "Ten seconds flat — " },
  "Bottom Line First": { b: "Bottom line first — ", c: "Bottom line first — " },
  "Conviction Floor": { b: "Hold the line — ", c: "Name your line — " },
  "Curiosity Lift": { b: "From real curiosity — ", c: "Curious, not defensive — " },
  ...
  "Draw the Line": { b: "Name your line — ", c: "Draw the line — " },
  "First Principles": { b: "From first principles — ", c: "Strip it down — " },
  "Make It Real": { b: "Skip the theory — ", c: "Skip the theory — " },
  "Metronome": { b: "Steady tempo — ", c: "Hold steady — " },
  "Monroe's Motivated Sequence": { b: "Attention → action — ", c: "Walk it through attention → action — " },
  ...
  "Order of Magnitude": { b: "Order of magnitude — ", c: "Is it 1×, 10×, 100× — " },
  "Pivot Tone": { b: "Pivot tone mid-answer — ", c: "Shift tone halfway — " },
  "Punctuation Breathing": { b: "Breathe with the punctuation — ", c: "Breathe at the commas — " },
  "Question Then Answer": { b: "Ask, then answer — ", c: "Pose the question, then answer — " },
  "Read the Room": { b: "", c: "" },
  "Signpost First": { b: "Signpost first — ", c: "Tell them the shape first — " },
  "Slow Cooker": { b: "Slow it down — ", c: "Slow your tempo — " },
  "Subject Verb Object": { b: "One clean sentence — ", c: "Subject, verb, object — " },
  "Tempo Shift": { b: "Shift tempo — ", c: "Shift tempo mid-answer — " },
  "The 3 Point Rule": { b: "Three reasons — ", c: "Three reasons, not four — " },
  "The Claim and Proof": { b: "Claim plus one proof — ", c: "Claim, then proof — " },
  "The Monotone Breaker": { b: "Find the energy — ", c: "Break the monotone — " },
  "The Perspective Shift": { b: "From their seat — ", c: "Shift to their view — " },
  "The Problem Solution Frame": { b: "Problem, then fix — ", c: "Name problem, name fix — " },
  "The Story Arc": { b: "Beginning, middle, end — ", c: "Tell it with an arc — " },
  "The So What Test": { b: "Make the point, then so what — ", c: "Pass the so-what — " },
  ...
  "Compare and Contrast": { b: "Compare two versions — ", c: "Compare and contrast — " },
};
...
function pickFromPool(pool, exIdx, goalIdx) {
  // Diagonal pick to maximize variety across exercises within a goal
...
function buildAuthored(exercises) {
  ...
  for (const [exIdx, ex] of exercises.entries()) {
    ...
    for (const [goalIdx, goal] of GOALS.entries()) {
      ...
      const aText = universalPool[goalIdx]
        || universalPool[goalIdx % Math.max(1, universalPool.length)]
        || `What's true about ${goal.replace(/_/g, " ")}?`;
      ...
      for (const tr of triples) {
        ...
        list.push(makePrompt(tr.text, tr.difficulty, ["finance", goal]));
      }
    }
...
// Load existing catalog and dedupe against existing text ---------------------
...
for (const ins of inserts) {
  ...
  for (const p of ins.prompts) {
    ...
    if (existingText.has(key)) {
      const goalTag = p.tags.find(t => t !== "finance") || "fin";
