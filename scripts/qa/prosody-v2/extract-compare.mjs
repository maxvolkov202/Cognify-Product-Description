...
import { env, sql, maskEmail, OUT_DIR, pctl, isTestEmail, isRealRep } from "./db.mjs";
...
const reps = await sql`
  select r.id, r.audio_url, r.duration_ms, r.created_at, r.model_version, u.email
...
const targets = reps.filter((r) => ("test-only" in args ? isTestEmail(r.email) : "real-only" in args ? isRealRep(r) : true));
...
// Per-feature drift tolerance, matched to each feature's scale (a single absolute
// epsilon is both too tight for Hz and too loose for ratios near curve knees).
const DRIFT_TOL = { pitchMeanHz: 2, pitchStdSemitones: 0.1, pitchRangeSemitones: 0.25, monotoneRatio: 0.02, upspeakRatio: 0.02, rmsMean: 0.5, rmsStd: 0.5, articulationScore: 0.02 };
const rows = [];
for (const r of targets) {
  ...
  rows.push({
    rep_id: r.id, email: maskEmail(r.email), test: isTestEmail(r.email), real: isRealRep(r), audio_path: r.audio_url, created_at: r.created_at,
    ...
    cache_drift: cached && a.features ? Object.fromEntries(NUMERIC.filter((k) => cached[k] != null && a.features[k] != null && Math.abs(cached[k] - a.features[k]) > DRIFT_TOL[k]).map((k) => [k, { cached: cached[k], fresh: a.features[k] }])) : null,
  });
  console.log(`${r.id.slice(0, 8)} ${isTestEmail(r.email) ? "test" : isRealRep(r) ? "real" : "mock"} A:${a.features ? "ok" : a.error} ${a.latency_ms}ms${b ? ` B:${b.features ? "ok" : b.error} ${b.latency_ms}ms` : ""}`);
}
