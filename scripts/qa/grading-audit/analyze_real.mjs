...
const scored = reps.filter(r=>r.composite_score!=null && !["seed-demo-v1","mock-fallback-v1"].includes(r.model_version));
...
fs.writeFileSync(OUT+"sample25_real.json", JSON.stringify(sample,null,2));
...
fs.writeFileSync(OUT+"analysis_real.json", JSON.stringify(out,null,1));
await sql.end();
