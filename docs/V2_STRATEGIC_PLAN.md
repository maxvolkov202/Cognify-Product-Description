...
- **2026-04-23 · CTO-agent (Claude)** · WS-9 initial pass shipped (5 commits): (1) `/about/team` new route with 5 named advisor bios (Hupe, Nahamoo, Ellen, Jeffrey, David) + gradient-initial avatars pending real headshots, (2) `/about` hero got a "Team & advisors" CTA, Footer got "Team & advisors" link and dropped "L&D teams" → "Teams", (3) `layout.tsx` meta keywords: removed banned "AI coach" + "L&D", added "speaking reps" + "brain productivity" + "corporate health", (4) `/for-teams` Corporate Health reframe — hero kicker + sub rewritten around brain productivity; valueProps expanded to 6 with Hupe/Nahamoo/David attribution; Kraft tribal-knowledge-loss + gym/stipend analogies both surface, (5) companion edits on `/pricing` Companies tier, `/use-cases` second case study, `EnterpriseCTA` kicker + progress-reporting body. Knowledge-base MDs still contain L&D in worked examples — educational content, not marketing surface.
- **2026-04-23 · CTO-agent (Claude)** · Polish: `/progress` now reads `RUBRIC_VERSION` from `rubric.ts` instead of a stale hard-coded string. Added "Words we use" glossary section on `/how-it-works` with 10 Cognify-specific terms — Rep, Rep type, Daily Workout, Build a Rep, Callout, Composite score, Structural adherence, Baseline rep, External validation, Rubric version. Closes the Product Sweep cross-cutting "jargon creep" finding. Documented in-place that the Progress Sweep Journey 6 items (improvement banner + /validate bridge) were already shipped — no code change needed.
- **2026-04-24 · CTO-agent (Claude)** · **WS-3 Pressure System shipped** on branch `feat/gym-core` (commits 249fe5d1 + 09319f5e). Tim Tebow made structural: every Daily Workout of 4+ reps now gets exactly one pressure rep at position N-1. 5 archetypes (Pushback / Time Compression / Audience Switch / Clarifying Interrupt / Stakes Raise) with 41 authored prompts spanning 7 professional domains; weight profiles per archetype (pending WS-4 consumption); PressureRepIndicator UI with amber/orange/rose accent palette; `handle_pressure` rep type promoted to pressure-aware; orchestrator rewritten to inject pressure slot; archetype rotation excludes previous-session archetype. 12 orchestrator tests pass. Design doc at `docs/proposals/pressure-system.md`.
- **2026-04-24 · CTO-agent (Claude)** · **WS-6 Session Types shipped** on `feat/gym-core` (commits 63a1c608 + 4008c35a). Three explicit session types: Focus / Combined / Flow. Focus drills one dimension across 4 reps (tier-1 primary, tier-2 secondary, fallback to others); pressure rep at N-1 preserved. Combined is existing behavior, now tagged. Flow is new — always 5 reps, every rep is a pressure rep, archetypes in a fixed intensity ramp (time_compression → audience_switch → pushback → clarifying_interrupt → stakes_raise) based on deliberate-practice literature. FlowFeedbackPanel: compressed single-insight panel with auto-advance (~4s, user-tappable), `prefers-reduced-motion` respected, insight prefers improvement callout over positive over neutral. SessionTypePicker on WorkoutIntro with Focus sub-picker for dimension. Preference persists via `cognify_session_type_v1` localStorage key. WorkoutEnd branches on sessionType — Flow gets a dedicated "Flow trajectory" section showing the 5-archetype ramp with per-rep composites. 42 tests pass. Design doc at `docs/proposals/session-types.md`. Branch deployed to `cognify-v2-neon.vercel.app`.
- **2026-04-24 · CTO-agent (Claude)** · **Deep-work chain on `feat/gym-core`** (7 more commits — total 13 on the branch). (a) Scoring weight profiles on pressure archetypes now actually affect composite — pressureArchetypeId threaded through `/api/score` route with archetype weights taking priority over framework weights. (b) Pressure badge surfaces in FeedbackPanel ("You just trained · Pushback"). (c) Pressure performance card on `/progress` driven by new `getPressureRepStats` query (pressure reps identified by topic prefix `"Pressure · {name}"`). (d) Personal-best toast via new `getUserDimensionMaxes` query — detects new PBs on rep complete, shows subtle Trophy toast with emerald palette, respects reduced-motion, auto-dismisses. (e) Weekly narrative (WS-7) — `/api/weekly-narrative` route + `generateWeeklyNarrative` (Claude Sonnet) + `WeeklyNarrativeCard` on /progress with 12h localStorage cache, dim-deltas pills, "next focus" CTA. (f) WS-4 callout taxonomy: tightened scoring prompt with banned-phrase list + post-LLM sanitizer that rewrites any callout tripping "good job" / "nice work" / generic-praise filters to a neutral-but-specific fallback. (g) Tomorrow's focus now actually biases tomorrow's workout — new `getLastSessionWeakestDimension` query feeds `pickRepTypes` with `weakestDimensionBias` which adds +2.0 weight boost to rep types whose primary/secondary includes the weak dim. (h) RECORDING pill indicator during active recording (rose-600 pulse). (i) Framework-with-notes per section on pre-rep screen (ephemeral session state, 60-char input per section). (j) PWA manifest.json + `InstallPrompt` gated on 3+ reps + 14-day dismiss cooldown + iOS Safari fallback instructions. 54/54 unit tests still pass after all changes.
- **2026-04-24 · CTO-agent (Claude)** · **Continuation pass on `feat/gym-core`** (5 more commits — total 18). (k) Blind-ranking CTA at bottom of FeedbackPanel: gradient card pitches "3 friends rank your reps without seeing scores — human signal the algorithm can't fake" with next/Link to /validate/new. Only shown on persisted (non-guest) reps. (l) Streak chip on WorkoutIntro (mockup #1 parity): orange flame pill "N days streak". (m) Pressure analytics on monthly report: `getPressureRepStats` refactored to accept `{ since, until }` date-range options; `/progress/month/[yyyyMm]` scopes to in-month pressure reps. (n) **Six-skills always-visible bottom bar** (Direction.md CRITICAL UI): fixed to bottom of (app) layout, dimension chips with color-coded dots + score, collapsed mode with just 6 dots, persists collapse preference, hidden on /onboarding + /tutorial, mobile horizontal-scroll. (o) Dynamic per-rep emphasis via `SkillsFocusContext` provider + `SkillsFocusScope` component — WorkoutSession sets focus to current rep's primary + secondary dims during the rep phase; bar shows primary with glow ring + secondary with soft border + non-focused at 50% opacity. Full build passes (not just typecheck — caught `<a>` vs next/Link lint rule in production build).
