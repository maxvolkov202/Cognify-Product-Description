...
const SUGGESTIONS = [
  ...
  { href: "/scenario", label: "Scenario training" },
...
export default function NotFound() {
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <div className="brand-gradient-text mt-8 text-[140px] font-extrabold leading-none tracking-[-0.05em] tabular-nums md:text-[180px]">
        404
      </div>
      ...
      <p className="mt-3 max-w-md text-base leading-relaxed text-ink-600">
        You may have followed a stale link, or the URL you typed doesn&rsquo;t exist.
        Here&rsquo;s where you probably want to go.
      </p>
        ...
        <div className="mt-3 flex flex-wrap justify-center gap-2">
          {SUGGESTIONS.map((s) => (
            <Link
              key={s.href}
              href={s.href}
              className="rounded-full border border-ink-200 bg-white px-3 py-1.5 text-xs font-semibold text-ink-700 transition-colors hover:border-brand-lavender hover:text-ink-900"
            >
              {s.label}
            </Link>
          ))}
