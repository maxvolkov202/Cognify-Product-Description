...
type RepRowWithLegacy = RepRow & {
  detailedScores?: Record<string, number>;
  primaryFocus?: { title: string; nextStep: string };
  analysisMetrics?: { fillerWordCount?: number; wordCount?: number; hasStrongOpening?: boolean; frameworkCoveragePercentage?: number };
};
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 rounded-t-2xl px-6 py-4 flex items-center justify-between">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-600">{rep.created_at ? formatDate(new Date(rep.created_at)) : "—"}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          {/* Core Scores */}
          {(rep as RepRowWithLegacy).detailedScores && (
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Core Scores</h3>
              <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                {Object.entries((rep as RepRowWithLegacy).detailedScores!).map(([dimension, score]) => (
          ...
          {(rep as RepRowWithLegacy).primaryFocus && (
            <div className="bg-[#9D7BF5] bg-opacity-10 border-2 border-[#9D7BF5] rounded-xl p-5">
              ...
              <div className="bg-white rounded-lg p-4 border border-[#9D7BF5]/30">
                <p className="text-sm font-semibold text-[#9D7BF5] mb-1">
                  {(rep as RepRowWithLegacy).primaryFocus!.title}
                </p>
                <p className="text-sm text-gray-700">
                  {(rep as RepRowWithLegacy).primaryFocus!.nextStep}
                </p>
              </div>
            </div>
          )}
          ...
          {/* Performance Metrics */}
          {(rep as RepRowWithLegacy).analysisMetrics && (
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-xl border border-gray-200 p-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-gray-900">{(rep as RepRowWithLegacy).analysisMetrics!.fillerWordCount}</p>
                  <p className="text-xs text-gray-600 mt-0.5">Filler words</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-gray-900">{(rep as RepRowWithLegacy).analysisMetrics!.wordCount}</p>
                  <p className="text-xs text-gray-600 mt-0.5">Total words</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-gray-900">
                    {(rep as RepRowWithLegacy).analysisMetrics!.hasStrongOpening ? '✓' : '✗'}
                  </p>
                  <p className="text-xs text-gray-600 mt-0.5">Strong opening</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-gray-900">
                    {Math.round((rep as RepRowWithLegacy).analysisMetrics!.frameworkCoveragePercentage ?? 0)}%
                  </p>
