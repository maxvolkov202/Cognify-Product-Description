...
import { PerformanceTrendChart, type TrendMetric } from "./PerformanceTrendChart";
...
export function RepHistoryPage({ reps, onBack, onRepClick }: RepHistoryPageProps) {
  ...
  // Calculate summary stats (using rep.overall_score, rep.scenario only)
  const stats = useMemo(() => {
    if (reps.length === 0) {
      return {
        totalReps: 0,
    ...
    const overallScores = reps.map(r => r.overall_score ?? 0).filter(Boolean);
    ...
    return {
      ...
      averageScore: overallScores.length ? Math.round(overallScores.reduce((a, b) => a + b, 0) / overallScores.length) : 0,
      bestScore: overallScores.length ? Math.max(...overallScores) : 0,
  ...
  const uniqueScenarios = useMemo(() => {
    const scenarios = new Set(reps.map(r => r.scenario ?? "").filter(Boolean));
    return Array.from(scenarios).sort();
  }, [reps]);

  const uniqueFrameworks = useMemo(() => {
    const frameworks = new Set(reps.map(r => r.framework ?? "").filter(Boolean));
    return Array.from(frameworks).map(id => FRAMEWORKS.find(f => f.id === id)).filter(Boolean);
  }, [reps]);

  // Filter and sort reps (using rep.overall_score, rep.scenario only)
  const filteredAndSortedReps = useMemo(() => {
    let filtered = [...reps];

    if (filterScenario !== "all") {
      filtered = filtered.filter(r => (r.scenario ?? "") === filterScenario);
    }
  ...
  const getOverallScore = (rep: RepRow) => rep.overall_score ?? 0;
  ...
  return (
    <section className="py-12 px-6">
      <div className="max-w-6xl mx-auto space-y-8">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            ...
                            <span>•</span>
                            <span>{rep.created_at ? formatDate(rep.created_at) : "—"}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        {/* Pattern Insights - temporarily commented out */}
