...
export default async function RepDetailPage({ params }: { params: Params }) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-5 px-6 py-10 md:py-12">
      <Link
        href="/progress"
        className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-ink-500 hover:text-ink-900"
      >
        <ArrowLeft className="size-3" strokeWidth={2.5} />
        Back to progress
      </Link>

      <section className="rounded-3xl border border-ink-200 bg-gradient-to-br from-white via-brand-lavender/5 to-brand-magenta/5 p-6 md:p-8">
      ...
      <section className="rounded-3xl border border-ink-200 bg-white p-6 md:p-7">
        <p className="mb-5 text-[11px] font-bold uppercase tracking-[0.14em] text-ink-500">
          Six core skills
        </p>
      ...
      {lead && (
        <section className="rounded-3xl border border-brand-purple/20 bg-brand-purple/5 p-6">
          ...
          {lead.quote && (
            <p className="mt-3 rounded-lg border-l-2 border-brand-purple/40 bg-white p-3 text-xs italic text-ink-700">
              &ldquo;{lead.quote}&rdquo;
            </p>
          )}
          {lead.suggestedRewrite && (
            <p className="mt-2 rounded-lg border-l-2 border-emerald-300 bg-emerald-50/60 p-3 text-xs text-ink-700">
