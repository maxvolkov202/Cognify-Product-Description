...
export function renderTextSignalsBlock(signals: TextSignals | null): string | null {
  ...
  return [
    "SIGNALS (text-derived surface measurements — corroborating evidence for the four content dimensions, NOT ground truth):",
    ...
    `  Structure: transitions ${s.transitionMarkerCount} (rate ${s.transitionMarkerRate}/min, target ≥3 per minute) | opening-position ${s.openingPositionScore}/100 | hierarchy markers ${s.pointHierarchyMarkers} | arc opening=${arc.clearOpening ? "Y" : "N"} middle=${arc.developedMiddle ? "Y" : "N"} close=${arc.definitiveClose ? "Y" : "N"} | logical flow ${s.logicalFlowScore}/100 (target ≥60) | coherence ${s.coherenceIndex}/100 (target ≥70)`,
    `  Conciseness: hedge ${cn.hedgeRatePerMinute}/min (target <1) | n-gram repetition ${cn.repetitionScore} (target <0.20) | words-per-distinct-idea ${cn.wordsPerDistinctIdea} | stopping-point accuracy ${cn.stoppingPointAccuracy}/100 (target ≥70 = clean stop)`,
    `  Thinking Quality: claim-support ${(t.claimSupportRate * 100).toFixed(0)}% (target >70%) | counterargument ${t.counterargumentMarkers} | depth markers ${t.depthOfAnalysisMarkers} | honesty markers ${t.intellectualHonestyMarkers} | originality ${t.originalityIndex}/100 (target ≥55) | self-correction count ${t.logicalConsistencyMarkers} (target 0; high = confused thinking → LOWER honesty)`,
    "",
    "How to use the SIGNALS: the TRANSCRIPT is your primary evidence; these numbers are pattern-matched approximations that only detect EXPLICIT surface markers (\"first/because/therefore\"). They reliably catch what IS flagged (a high hedge rate or jargon rate is real) but routinely MISS implicit structure, narrative arcs, and evidence given without connective words — a tight pitch can show \"logical flow 9/100\" while being impeccably organized. Never lower a dimension below what your own reading of the transcript supports just because a signal number is low; when your score and a signal clearly disagree, trust the transcript and name the discrepancy in that dimension's `signals` array (e.g. \"transition rate 0/min, but the speaker uses implicit narrative structure end-to-end\").",
  ].join("\n");
}
